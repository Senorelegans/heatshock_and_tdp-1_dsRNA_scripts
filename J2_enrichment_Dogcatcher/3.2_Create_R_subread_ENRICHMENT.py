...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Part II: Calculating DOGS for sense and antisense strands.')
    ...
    parser.add_argument('--control_TOTAL_BAM_list', nargs="*", action= 'store', metavar='input_path_BAM_folder', help='List of files for your control samples')
    parser.add_argument('--treatment_TOTAL_BAM_list', nargs="*", action= 'store', metavar='input_path_BAM_folder', help='List of files for treated samples')
    parser.add_argument('--control_J2_BAM_list', nargs="*", action= 'store', metavar='input_path_BAM_folder', help='List of files for your control samples')
    parser.add_argument('--treatment_J2_BAM_list', nargs="*", action= 'store', metavar='input_path_BAM_folder', help='List of files for treated samples')
    ...
    bamlist = str(control_TOTAL_BAM_list + treatment_TOTAL_BAM_list + control_J2_BAM_list + treatment_J2_BAM_list).strip("[").strip("]")


    ###Generate col data file
    control_TOTAL_BAM_list = [ "X." + bam.replace("/",".").replace("-",".").replace("..",".") + "\tC" + "\tT"for bam in control_TOTAL_BAM_list ]
    treatment_TOTAL_BAM_list = [ "X." + bam.replace("/",".").replace("-",".").replace("..",".") + "\tT" + "\tT"for bam in treatment_TOTAL_BAM_list ]
    control_J2_BAM_list = [ "X." + bam.replace("/",".").replace("-",".").replace("..",".") + "\tC" + "\tJ" for bam in control_J2_BAM_list ]
    treatment_J2_BAM_list = [ "X." + bam.replace("/",".").replace("-",".").replace("..",".") + "\tT"+"\tJ" for bam in treatment_J2_BAM_list ]
    

    with open(output_prefix + "/col_data.txt", "w") as outfile:
        outfile.write("\tcondition" + "\tassay"+ "\n")
        for bam in control_TOTAL_BAM_list:
            outfile.write(bam + "\n")
        for bam in treatment_TOTAL_BAM_list:
            outfile.write(bam + "\n")
        for bam in control_J2_BAM_list:
            outfile.write(bam + "\n")
        for bam in treatment_J2_BAM_list:
            outfile.write(bam + "\n")
    ...
    def create_R_script():
        with open(input_R_template_file, "r") as infile, open(output_prefix + "/" + "Rsubread_initial.R", "w") as outfile:
            for line in infile:
