...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Part 3.3: Normalizing Calculating DOG for sense and antisense strands.')
    ...
    #Write out normalization ratio to divide genes by

    TOJ2_LIST = TOTAL_LIST+J2_LIST
    print(TOJ2_LIST)
    NOMAPPED_LIST = input_nomapped_ratio_list+j2_nomapped_ratio_list
    with open(input_prefix+"rRNA_subtraction_ratio.txt","w") as outfile:
        #outfile.write("\t".join(TOJ2_LIST)+"\t".join(NOMAPPED_LIST))
        x = 0
        ...
        outfile.write("\n")
        for num in input_nomapped_ratio_list:
            outfile.write(str(num)+"\t")
        for num in j2_nomapped_ratio_list:
            outfile.write(str(num)+"\t")

    ##############
    #2. Apply normalization
    filelist = [input_prefix + "/Rsubread_antisense.txt", input_prefix + "/Rsubread_sense.txt"]
    for f1 in filelist:
        fN = f1[:-4]
