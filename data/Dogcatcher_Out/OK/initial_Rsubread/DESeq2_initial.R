...
cts <- as.matrix(read.csv("OK_LFC_FLIPPED_DOG_2ndtime/initial_Rsubread/Rsubread_sense_normalized_rRNA_star.txt",sep="\t",row.names=1))
...
res <- results(dds)
write.csv(res, file="OK_LFC_FLIPPED_DOG_2ndtime/initial_Rsubread/DESeq2_sense.csv")
#############################################
...
cts <- as.matrix(read.csv("OK_LFC_FLIPPED_DOG_2ndtime/initial_Rsubread/Rsubread_antisense_normalized_rRNA_star.txt",sep="\t",row.names=1))
...
dds <- DESeq(dds, test="LRT", reduced= ~ assay + condition, parallel = TRUE)
res <- results(dds)
write.csv(res, file="OK_LFC_FLIPPED_DOG_2ndtime/initial_Rsubread/DESeq2_antisense.csv")
#############################################
