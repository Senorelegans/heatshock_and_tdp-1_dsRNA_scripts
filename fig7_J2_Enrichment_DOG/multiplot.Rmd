...
mp = multiplot(hs, ok, hsa, oka, cols=2)
...
dev.copy(png,filename="DOG_ADOG_PLOT_noOperon.png",width=700, height=900)
...
df <- read.table("~/Google_Drive/Scripts/hs/worm/EMBO/writeup/Figures/fig7_J2_Enrichment_DOG/unique_DOG/brunquell_combined_DOG_unique_cleaned.csv", header=TRUE,sep=",")
...
data <- df
  ...
  # Color corresponds to fold change directionality
  bhs <- ggplot(data, aes(x = log2baseMean, y = log2FoldChange)) +
    geom_point(aes(color = factor(color)), size = 1.75, alpha = 0.8, na.rm = T) + # add gene points
    xlim(4.3, 20) +
    theme_bw(base_size = 15) + # clean up theme
    theme(legend.position = "none") + # remove legend 
  ...
  # Plot figure 
  bhs
    file="~/Google_Drive/Scripts/hs/worm/EMBO/writeup/Figures/fig7_J2_Enrichment_DOG/unique_DOG/brunquell_combined_DOG_unique_cleaned.csv.png"
  ggsave(file=file, device = "png")
  dev.off()
...
df <- read.table("~/Google_Drive/Scripts/hs/worm/EMBO/writeup/Figures/fig7_J2_Enrichment_DOG/unique_DOG/brunquell_combined_ADOG_unique_cleaned.csv", header=TRUE,sep=",")
...
data <- df
  ...
  # Color corresponds to fold change directionality
  bhsa <- ggplot(data, aes(x = log2baseMean, y = log2FoldChange)) +
    geom_point(aes(color = factor(color)), size = 1.75, alpha = 0.8, na.rm = T) + # add gene points
    xlim(4.3, 20) +
    theme_bw(base_size = 15) + # clean up theme
    theme(legend.position = "none") + # remove legend 
  ...
  # Plot figure 
  bhsa
    file="~/Google_Drive/Scripts/hs/worm/EMBO/writeup/Figures/fig7_J2_Enrichment_DOG/unique_ADOG/brunquell_combined_ADOG_unique_cleaned.csv_labelled.png"
  ggsave(file=file, device = "png")
  dev.off()
...
mp = multiplot(bhs, bhsa, cols=1)
dev.copy(png,filename="~/Google_Drive/Scripts/hs/worm/EMBO/writeup/Figures/fig7_J2_Enrichment_DOG/DOG_BrunquellHS_sense_antisense.png")
dev.off()
```
